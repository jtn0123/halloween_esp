...
#### A1 — The restart-race fix landed only in the non-shipping Rust studio
...
#### A2 — The Rust studio hardcodes `.venv/bin/python`
...
#### A3 — netguard is the only port with no cross-language differential gate — and it's the SSRF guard
...
#### A4 — PARITY.md omits five of the six duplicated-logic pairs Rust created
...
#### A5 — `POST /studio/compare` reports the caller's typo as a 500, in both languages
...
#### A6 — Stale Rust module headers describe several passes ago
...
#### A8 — `scenes/scenes.yaml` is 487 lines against the hard 500 cap
...
#### B1 — Zero-byte file + Range → `Content-Length: 1`, nothing written, connection desynced
...
#### B2 — Request bodies buffered ~3× in RAM; connection buffer never shrinks
...
#### B3 — Decode LRU bounded by count, not bytes — up to ~2.5 GB resident
...
#### C1 — Prior C2 regressed: three files within 32 lines of the hard cap
...
#### C2 — The generated page has no DOCTYPE and no `lang` — quirks mode + WCAG 3.1.1 failure
...
#### C3 — The scrub slider has no `aria-valuemax`
...
#### C4 — `dom_discipline` guard misses `querySelector`; `dom.ts` has no scoped helper
...
#### C5 — 92 non-null assertions, 12 of them re-creating the exact bug `req()` prevents
...
#### C6 — Seven dead exports invisible to `noUnusedLocals`
...
#### D1 — CI has never validated the Rust port: parity diverges on Linux, wasm target missing, clippy red
...
#### D2 — Rust test/fmt/clippy gates are transitively gated on a C++ compiler
...
#### D3 — `make test-fast` now triggers two full Rust release builds + clippy + twelve server spawns
...
#### D4 — The Rust studio half (≥2,700 lines) has zero unit tests
...
#### D5 — The 148-test e2e gate against the Rust studio ran once, by hand
...
#### D6 — `free_port()` TOCTOU across two servers
...
#### D7 — The enforced coverage number describes a shrinking fraction of shipped code
...
#### E1 — `panic = "abort"` makes the studio's documented error boundary dead code
...
#### E2 — `.claude/worktrees/` (675 MB, other projects' full checkouts) ignored only per-clone
...
#### F1 — The esphome pin lives in three places; the two blocked PRs must land as one
...
#### F3 — No `rust-toolchain.toml`; clippy `-D warnings` floats on the runner's rustc
...
#### F4 — No `cargo` ecosystem in dependabot.yml
...
#### G1 — The Rust studio is finished, parity-gated, and unreachable by default
...
#### G2 — The page budget was raised once and will be breached again in ~2 weeks
...
#### G3 — See B6 (cargo no-op build on every render/import child; uncached in CI)
- **Effort:** S
...
#### H1 — README: setup path broken, diagram pre-B3, "four implementations" now five
...
#### H2 — CLAUDE.md line 6 tells every agent Rust does not apply to this repo — and the previewer paragraph argues the opposite of current behavior
...
#### H3 — CONTRIBUTING's setup cannot produce a green `make check`
...
#### H4 — API.md's party list and PARITY.md's runbook commands are stale
...
#### H5 — web/MIGRATION.md's "is now the actual layout" is 47 modules out of date
...
#### I1 — Zero Rust targets in the Makefile; `cargo` appears in no entry point
...
#### I2 — Pre-commit hook: no `cargo fmt --check`
...
#### I3 — No dedicated Rust CI job; Rust failures report as "coverage-gated unit tests failed"
...
#### I4 — `core/src/httpd.rs` (467) and `jsonio.rs` (453) are in the cap warning band, and the least-tested file is the closest one
