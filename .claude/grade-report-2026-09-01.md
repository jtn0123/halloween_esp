...
#### ~~A1~~ ✓ done 2026-09-01 — Feature-gate the crate; the wasm stubs are the evidence (was A7)
...
#### ~~A2~~ ✓ done 2026-09-01 — 58 comments cite bare grade-report item IDs that renumber every audit
...
#### ~~A3~~ ✓ done 2026-09-01 — `core/Cargo.toml` declares four of the eight bins
...
#### ~~A4~~ ✓ done 2026-09-01 — The `studio_publish ↔ studio_scenes` cycle is held open by an unannotated deferred import
...
#### ~~B1~~ ✓ done 2026-09-01 — Short read mid-serve under-delivers a promised Content-Length on keep-alive
...
#### ~~B2~~ ✓ done 2026-09-01 — `read_request`'s comment credits removing a memmove that's still there
...
#### ~~C1~~ ✓ done 2026-09-01 — `waveform.ts` (473) needs the DI refactor its closure shape demands (carryover)
- **Where:** one 393-line closure over shared mutable state; pure extraction can't clear the 440 bar.
- **Effort:** M

#### ~~C2~~ ✓ done 2026-09-01 — 3,562 lines of e2e specs are outside `tsc --noEmit`
...
#### ~~C3~~ ✓ done 2026-09-01 — Three DOM non-null assertions survived the sweep, one systematized
...
#### ~~C4~~ ✓ done 2026-09-01 — `device.ts` pokes `device_chip.ts`'s subtree eight times by raw querySelector
...
#### ~~D1~~ ✓ done 2026-09-01 — Ten parity-gate call sites run cargo from the repo root, silently bypassing the toolchain pin
...
#### ~~D2~~ ✓ done 2026-09-01 — 2,619 lines of the Rust studio layer still have zero `#[test]`
...
#### ~~D3~~ ✓ done 2026-09-01 — The two page ceilings can disagree by 285 KB and redden CI on a correct build
...
#### ~~D4~~ ✓ done 2026-09-01 — `tools/lock_deps.py` (new, 163 lines) has no tests
...
#### ~~D5~~ ✓ done 2026-09-01 — `web/package.json` maintains the suite list twice
...
#### ~~E1~~ ✓ done 2026-09-01 — A panic inside the Rust decode leaves a permanent in-flight marker; later requests for that track deadlock
...
#### ~~E2~~ ✓ done 2026-09-01 — Relayed header values reach `head()` unvalidated; the CRLF safety is accidental
...
#### ~~E3~~ ✓ done 2026-09-01 — No read timeout on studio connections
...
#### ~~F1~~ ✓ done 2026-09-01 — Nothing in CI resolves `requirements.txt`; Dependabot already walked through the gap
...
#### ~~F2~~ ✓ done 2026-09-01 — Close PR #13 and ignore aioesphomeapi majors until esphome moves
...
#### ~~F3~~ ✓ done 2026-09-01 — `firmware-weekly` is the one job with no Rust toolchain or cache
...
#### ~~F4~~ ✓ done 2026-09-01 — Edition 2021 → 2024 (deliberately deferred yesterday; do with A1)
...
#### ~~G1~~ ✓ done 2026-09-01 — Ship the Rust studio as the default (carryover; the CI gate now exists)
...
#### ~~G2~~ ✓ done 2026-09-01 — The waveform cache deep-clones thousands of JSON nodes per request under a mutex
...
#### ~~H1~~ ✓ done 2026-09-01 — CLAUDE.md describes the previewer budget the evening replaced
...
#### ~~H2~~ ✓ done 2026-09-01 — Two live env knobs are documented nowhere operator-facing
...
#### ~~H3~~ ✓ done 2026-09-01 — README counts are stale: "three" sandbox vars (now four), 7 of 10 scenes listed, `rust-coverage` absent from both target lists
...
#### ~~H4~~ ✓ done 2026-09-01 — The design record (`PROJECT_NOTES.md`, `docs/notes/`) has zero mentions of Rust
...
#### ~~H5~~ ✓ done 2026-09-01 — PARITY.md claims Linux x86_64 verification without naming the AVX-512 mechanism; API.md points at the wrong file for the route table
...
#### ~~I1~~ ✓ done 2026-09-01 — `tools/castle_fuzz.py` is at 499/500; the next one-line change reddens the hook
...
#### ~~I2~~ ✓ done 2026-09-01 — The grade reports are in the LOC cap's scope and two are within 20 lines of it
...
#### ~~I3~~ ✓ done 2026-09-01 — A soft warning band that fails the hook at 490 would keep 499 from recurring
