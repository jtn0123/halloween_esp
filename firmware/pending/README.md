...
Applied in v5.50 (2026-09-14):

- **Starting a scene hands the strips back to the Show effect**
  (`run_scene`, generated by `tools/gen_esphome.py`). A colour, a bench
  pattern or `off` from the desk's channel test takes the pixels off Show,
  and until now nothing gave them back before the next boot — found on the
  S3 Feather bring-up, where every scene ran dark after the static light
  test ended with `off`. Any zone that is off or on another effect is
  relit before the scene runs; `halt` (the `/api/play` path) leaves the
  lights alone as before. The emulator mirrors it. Both Feather builds and
  the carrier build change, since the generated scenes are shared.
...
- **`firmware/castle_feather_s3.yaml` — an ESP32-S3 Feather in the v3.3a
  ...
  2. First flash the BARE Feather, before it goes into the carrier: USB-C
     to the Mac, hold BOOT, tap RESET, release BOOT, then `make
     upload-fs3`. A factory Feather runs Adafruit's own USB stack, not the
     S3's ROM port. Once this image is on it, uploads and `make logs-fs3`
     need no buttons. In the carrier it is brick first, USB second, never
     USB alone: USB-only power drives 3.3 V logic into an unpowered
     74AHCT125 and both amps (castle-carrier-v3 docs/AUDIT-2026-09-03.md
     E-F4).
