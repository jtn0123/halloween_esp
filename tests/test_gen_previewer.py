...
class TestSceneYamlSlice(unittest.TestCase):
    ...
    def test_slice_starts_at_its_own_id(self) -> None:
        self.assertTrue(gp.scene_yaml_slice(self.TEXT, "beta")
                        .startswith("  - id: beta"))

    def test_trailing_whitespace_is_trimmed(self) -> None:
        self.assertEqual(gp.scene_yaml_slice(self.TEXT, "beta"),
                         "  - id: beta\n    name: Beta")
    ...
    def test_final_scene_needs_a_trailing_newline(self) -> None:
        ...
        self.assertIn("not found",
                      gp.scene_yaml_slice(self.TEXT.rstrip("\n"), "beta"))
class TestToPreviewer(unittest.TestCase):
    ...
    def test_documented_shape(self) -> None:
        s = scene(volume=0.6, loop=True, blurb="two\n  lines   here",
                  levels={"door": 0.5})
    ...
    def test_score_becomes_audio_bus_cues_in_milliseconds(self) -> None:
        ...
        self.assertEqual((cue["t"], cue["bus"], cue["op"], cue["snd"]),
                         (1500, "AUD", "play", "toll"))
    ...
    def test_wind_loops_only_in_a_looping_scene(self) -> None:
        ...
        sc = scene(loop=True, score=[{"t": 0, "synth": "wind"},
                                     {"t": 0, "synth": "organ"}])
    ...
    def test_set_cue_carries_zone_effect_and_level(self) -> None:
        s = scene(cues=[{"t": 40, "op": "set", "zone": "door",
                         "effect": "blood", "level": 0.25, "note": "turn"}])
        cue = gp.to_previewer(s, 1, "", {})["cues"][0]
        self.assertEqual(cue, {"t": 40, "bus": "LED", "op": "set",
                               "zone": "door", "eff": "blood",
                               "detail": "turn", "level": 0.25})
    ...
    def test_cues_come_out_in_time_order(self) -> None:
        ...
        s = scene(score=[{"t": 2.0, "synth": "toll"}],
...
class TestInjection(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self._saved = (gp.STYLES, gp.WEB, gp.BUNDLE, gp.subprocess)
    ...
    def tearDown(self) -> None:
        gp.STYLES, gp.WEB, gp.BUNDLE, gp.subprocess = self._saved
    ...
    def test_styles_replace_the_marker_comment(self) -> None:
        ...
        self.assertEqual(got, "<style>body { color: red }</style>")
    ...
    def test_bundle_replaces_the_marker_comment(self) -> None:
        self.assertEqual(gp.inject_bundle("<script>/* @BUNDLE x */</script>"),
