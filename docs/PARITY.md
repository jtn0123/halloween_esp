...
| Card cue file (a song's light show as `/sd/<track>.cue`, v5.63) and what each record DOES to the zone globals | `tools/cue_file.py` (writes it; `decode` and `loaded_count` are the reference) · `firmware/castle_cues.h` (reads it into PSRAM and applies it) · `tools/gen_esphome.py`'s cue lambdas (the same writes, as a script) | `tests/test_cue_file_cxx.py` compiles the real header (`tests/cxx/cues_check.cpp`), loads a file Python wrote, and compares the zone globals after every cue with a trace built from `decode` — plus every malformed file both must refuse; `tests/test_card_cues.py` holds `/api/status` `cues` in the emulator to `loaded_count` |
