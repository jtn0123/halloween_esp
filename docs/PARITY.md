...
| Castle wire protocol (`/api/*` on the device) | `firmware/sd_web.h` · `tools/castle_emu_wire.py` · `core/src/bridge.rs` (the `castle` bin, a client of the same wire) | `tests/test_firmware_contract.py` (routes, error strings, status keys) and `tests/test_firmware_names.py` (the byte rules) parse the C; `tests/test_bridge_rust.py` runs every verb against `castle_emu` |
