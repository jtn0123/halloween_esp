...
| Pulse → cue merge (zone routing, round-robin, velocity rounding) | `tools/gen_scene_cards.py` (what the card gets) · `tools/gen_previewer.py` (what the desk shows) | `tests/test_generator_parity.py`, `tests/test_gen_fuzz.py`, `tests/test_pulse_cap.py`. **Neither side thins any more**: `PULSE_CAP` was a limit on how many actions a generated scene script could hold, and v5.67 deleted the scripts — both sides now keep every hit, and `test_pulse_cap.py` asserts they keep the SAME ones |
...
| Card cue file — a song's light show as `/sd/<track>.cue` (v5.63) and a SCENE's as `/sd/scenes/<id>.cue` (v5.67, one reader for both) — and what each record DOES to the zone globals | `tools/cue_file.py` (writes it; `decode` and `loaded_count` are the reference) · `firmware/castle_cues.h` (reads it into PSRAM and applies it) | `tests/test_cue_file_cxx.py` compiles the real header (`tests/cxx/cues_check.cpp`), loads a file Python wrote, and compares the zone globals after every cue with a trace built from `decode` — plus every malformed file both must refuse; `tests/test_card_cues.py` holds `/api/status` `cues` in the emulator to `loaded_count`; `tests/test_scene_cue_equivalence.py` is the gate that let the scene scripts go — for every scene in scenes.yaml it compares the deleted script generator's timeline with the cue records |
| The card's scene manifest (`/sd/scenes/show.man`, v5.67) — what a generated script used to hard-code: id, audio token, volume, length, loop flag | `tools/scene_manifest.py` (`MAGIC`/`VERSION`/`HEADER`/`ENTRY`, `encode`/`decode`, and the four refusals) · `firmware/castle_scenes.h` (`Header`/`Entry`, `open_manifest`, `find`) · `tools/castle_emu_scenes.py` (`card_scene_ids` — what `/api/status`'s `scenes` says) | `tests/test_scene_manifest_cxx.py` compiles the real header (`tests/cxx/scenes_check.cpp`), writes a card with Python and drives the runner over it — every number through the round trip, the state machine (start, loop at length, stop, restart mid-scene, scene→raw track→scene, missing cue → fallback) and eight malformed manifests BOTH sides must refuse identically; `tests/test_firmware_contract.py` holds the two layouts and the scene ceiling equal by parsing the C |
...
| Import URL policy (which hosts yt-dlp may be handed) | `tools/netguard.py` · `core/src/netguard.rs` (the `netguard_dump` bin, `core/src/bin/netguard_dump.rs` — a URL corpus and a DNS table on stdin, one verdict per line out) | `tests/test_netguard_rust.py` drives both over the corpus `tests/test_netguard.py` holds the Python to, DNS mocked from one table on both sides, and compares the **refusal sentences**, not just the verdicts — the desk shows the string. The corpus carries one row per IPv6 range CPython calls non-global — `2002::/16`, `2001::/23`, `2001:db8::/32`, `3fff::/20`, `100::/64` — as a name AND as a literal, plus both NAT64 faces (`64:ff9b::7f00:1` refused, `64:ff9b::808:808` allowed, `64:ff9b:1::/48` never) and a name that resolves to nothing, which fails CLOSED for a LAN caller and is still exempt for loopback: the ranges a `2000::/3` shortcut got wrong (grade report 2026-09-17 pm E1) |
