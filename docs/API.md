...
| GET | `/api/events` | (v5.59) the event ring: what the MAIN LOOP did, oldest first, 64 entries deep and never growing — `[{t, e, a}]`, `t` = uptime ms, `e` = `play`/`scene`/`stop`/`volume`/`show`/`blackout`/`restart`/`light_evicted`/`sound`/`silent`, `a` = the command's argument (a track name is kept to 47 bytes, and v5.62 puts `"trunc":true` beside one that was cut — present only when it happened, so an ordinary line is unchanged; without it /api/events named a song that does not exist). A page polling `/api/status` once a second cannot see a scene that started and stopped in between; this is the record of the ticks it missed. `sound` is the tick the amplifier got its first sample and `silent` the tick playback ended — a command whose audio never arrives shows `silent` with no `sound` before it, which is what a scene whose track is not on the card looks like |
