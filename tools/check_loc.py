...
EXEMPT_PATHS: dict[str, str] = {
    ...
    "firmware/generated/fallback_scenes.h": "generated by tools/gen_esphome.py from scenes/scenes.yaml",
    ...
    # 113 pins and 2,460 digests: every wheel of every version, for every
    # platform the lock is installed on, so CI can install --require-hashes.
    # The length is the number of files PyPI published, not a seam anyone
    # could split on.
    "requirements.lock": "pip lockfile, generated by tools/lock_deps.py — a pin plus one hash line per released file",
    # Recorded answers, not prose: one JSON object per route or refusal, and
    # the file grows by whatever the corpus covers. There is no seam to split
    # them on — half a golden is not a golden — and the cap would otherwise
    # put a `make check` failure between adding a case and recording it.
    "tests/golden/read_routes.json": "generated by tools/gen_golden.py — the studio's recorded read-route answers",
    "tests/golden/scene_errors.json": "generated by tools/gen_golden.py — the studio's recorded scene-splice refusals",
}
