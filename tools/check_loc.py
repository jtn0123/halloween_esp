...
EXEMPT_PATHS: dict[str, str] = {
    "firmware/generated/scenes.yaml":
        "generated by tools/gen_esphome.py from scenes/scenes.yaml",
    "previewer/castle-cue-desk.html":
        "generated by tools/gen_previewer.py (the whole cue desk, audio inlined)",
    "audio/markers.json":
        "generated by tools/render_audio.py — beat/onset markers for every scene",
    "web/package-lock.json":
        "npm lockfile, written by npm install",
}
...
def measure(root: Path = ROOT, files: list[Path] | None = None,
...
def main() -> int:
    ...
    if not over:
        ...
        print(f"LOC check PASS — {len(rows)} files in scope, largest "
              f"{biggest[0]} lines ({biggest[1]}), cap {LIMIT}")
        print(f"  scope: {scope_summary(rows)}")
        print(f"  exempt: {len(EXEMPT_PATHS)} generated files "
