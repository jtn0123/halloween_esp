...
# The strips above are written once, for the porch board. The ESP32-S3
# carrier build includes them and then amends the ONE number that belongs
# to the chip rather than to the rig — its RMT block is 48 words, not 64
# (docs/V5-SPEC.md §13.4). See gen_rig.emit_rmt_override.
LIGHTS_S3_OUT = bp.GENERATED / "lights_s3.yaml"
...
def main() -> int:
    ...
    out.extend(emit_dispatch(doc, zones, script_ids))
    ...
    LIGHTS_S3_OUT.write_text(emit_rmt_override(layouts, zones, CHIPS["esp32s3"]))
    ids = ",".join(f"'{s['id']}'" for s in doc["scenes"])
    FALLBACK_SCENES_OUT.write_text(
        "// GENERATED BY tools/gen_esphome.py — DO NOT EDIT\n#pragma once\n"
        f'inline constexpr const char kFallbackSceneIds[] = "{ids}";\n'
    )

    print(
        f"wrote {bp.rel(OUT)} + {AUDIO_SD.name} + {RIG_OUT.name} + "
        f"{LIGHTS_OUT.name} + {LIGHTS_S3_OUT.name} + {FALLBACK_SCENES_OUT.name}"
    )
