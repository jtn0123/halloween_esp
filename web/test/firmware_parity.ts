 ...
 *     PARITY_SEED=9 PARITY_CASES=20000 … to go hunting
 *
...
const NOISE: Record<string, { w: number; k: number; gain: number }> = {
...
/** One JSON line from the firmware dump; which fields are live depends on `kind`. */
interface Row {
  kind: string; zi: number; n: number; center: number; fall_steps: number;
...
const zones = [...zoneBlock.matchAll(/\{([^{}]*)\}/g)].map((m) => {
  ...
  return id && fixture
    ...
    : null;
}).filter((z): z is { id: string; fixture: string; pixels: number | undefined } => z !== null);
...
interface Stat { n: number; max: number; sum: number }
const stat = (): Stat => ({ n: 0, max: 0, sum: 0 });
const add = (s: Stat, d: number): void => { s.n++; s.sum += d; if (d > s.max) s.max = d; };
const maxDiff = (a: readonly number[], b: readonly number[]): number =>
  Math.max(...[0, 1, 2, 3].map((c) => Math.abs(a[c]! - b[c]!)));
const f = (v: number): string => v.toExponential(2);
...
const noiseTol = (x: number, gain = 1): number => 2e-4 + 12 * Math.abs(x) * F32_EPS * gain;
...
for (const r of rows.filter((r) => r.kind === "noise")) {
  ...
  nd.exact += hi === r.hashi && h3 === r.hash3 ? 1 : 0;
...
const uniform = (label: string, sample: (i: number) => number, n = 65536): number => {
  const buckets = new Array<number>(16).fill(0);
  let sum = 0;
  for (let i = 0; i < n; i++) {
    const v = sample(i);
    sum += v;
    const b = Math.floor(v * 16);
...
const smoothTol = (w: number, t: number): number =>
  (t < 60 ? 2e-4 : 2e-4 + 4 * Math.abs(w * t) * F32_EPS);
/** True when the phase sits within float32 error of a branch edge. */
const nearEdge = (phase: number, edgeDist: number): boolean =>
  edgeDist <= 8 * Math.abs(phase) * F32_EPS + 1e-6;

for (const r of rows.filter((r) => r.kind === "px")) {
  const name = NAMES[r.eff]!;
  const P = { ...defaultParams(), hue: r.hue, soft: r.soft === 1, pal: r.pal };
  const L = layouts[r.zi]!;
  const base = (EFFECTS as Record<string, EffectFn>)[name]!(r.t, r.seed, P);
  for (let c = 0; c < 4; c++) {
  ...
  if (r.ov === 1) {
    ...
    sparkle.fwLit += fwLit ? 1 : 0;
    sparkle.deskLit += deskLit ? 1 : 0;
    sparkle.both += fwLit === deskLit ? 1 : 0;
  ...
  if (r.ov === 2) {
    ...
    ok(dOvl <= 2e-4 + 4 * L.n * Math.abs(ph) * F32_EPS + 1e-3 * (r.t >= 60 ? 1 : 0),
  ...
  if (r.ov === 3) {
    ...
    else ok(dOvl <= 2e-4 + 10 * Math.abs(ph) * F32_EPS + 1e-3 * (r.t >= 60 ? 1 : 0),
