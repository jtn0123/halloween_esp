...
const { createState, step } = await import("../src/show.js");
const { defaultParams } = await import("../src/effects.js");
...
interface TCue {
  t: number; bus: string; op: string;
  snd?: string; zone?: string; eff?: string; ms?: number; targets?: string[];
}
const A_CUES: TCue[] = [
...
const A = scene({ id: "a", dur: 4000, cues: A_CUES });
const B = scene({ id: "b", dur: 2000, loop: true, cues: B_CUES });
...
const fired: { snd: string; at: number }[] = [];
const synthPlays: [number, string][] = [];
synth.play = (name: string) => synthPlays.push([clock.now(), name]);
...
for (const f of fired) {
  const due = A_CUES.find(c => c.snd === f.snd)!.t;
...
/* ── Scene change mid-show: the new scene's cues, from ITS zero ─────── */
{
  ...
  ok(tick !== undefined && tick.at >= 300 && tick.at < 300 + FRAME,
...
/* ── The latency slider moves light→sound and light→file together ──── */
{
  const r2 = new Transport({
    state, rendered,
    synth: synth as unknown as TransportDeps["synth"],
    getMode: () => "rendered", onSceneChange: () => {} });
  r2.loadScene(A);
